...
def train_sequential_model(num_iterations=10000, d=0.01, data_tensors=None, patience=1000):
    ...
    # Loss weights
    lambdas = [1.0 , 0.1, 0.01, 0.001, 0.0001, 0.00001]
    ...
    with tqdm(total=num_iterations, desc="Sequential Training") as pbar:
        while i <= num_iterations:
            ...
            for j in range(30):
                ...
                u1_loss = lambdas[1] * u1_loss1 + lambdas[0] * (u1_loss2 + u1_loss3)
                u1_loss.backward()
                optimiser1.step()
            ...
            for j in range(30):
                ...
                u2_loss = lambdas[4] * (u2_loss1 + u2_loss2) + lambdas[1] * u2_loss3
                u2_loss.backward()
                optimiser2.step()
            ...
            for j in range(30):
                ...
                u3_loss = lambdas[1] * (u3_loss1 + u3_loss2)
                u3_loss.backward()
                optimiser3.step()
...
def run_grid_search(max_iterations=10000, patience=1000):
    ...
    hyperparameter_grid = {
        'n_hidden': [16, 32, 64],          # Number of neurons per hidden layer
        'n_layers': [3, 4, 5],             # Number of hidden layers
...
def main():
    ...
    # Print data summary
    print(f"\nData summary:")
    print(f"Time range: {t_tensor.min().item():.4f} to {t_tensor.max().item():.4f}")
    print(f"u1 range: {u1_tensor.min().item():.4f} to {u1_tensor.max().item():.4f}")
    print(f"u2 range: {u2_tensor.min().item():.4f} to {u2_tensor.max().item():.4f}")
    print(f"u3 range: {u3_tensor.min().item():.4f} to {u3_tensor.max().item():.4f}")
    ...
    best_result = run_grid_search(max_iterations=1, patience=1000)
    print(f"\nGrid search completed. Best model saved with MSE: {best_result['total_mse']:.6f}")
    ...
    # Option 4: Train a simultaneous model (placeholder)

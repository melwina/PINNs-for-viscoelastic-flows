...
# Function to find optimal loss weights
def find_optimal_weights(file_path=None, seed=1234, layers=5, neurons=64, max_epochs=4000):
    # Create plots directory if it doesn't exist
    plots_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "efc_plots")
    if not os.path.exists(plots_dir):
        os.makedirs(plots_dir)
    ...
    with tqdm(total=len(weight_configs), desc="Weight optimization") as pbar:
        for weights in weight_configs:
            pbar.set_description(f"Training with weights={weights}")
            ...
            model, history = train_model(
                ...
                batch_size=512,
                use_lbfgs=True,
                patience_adam=500,  # Use early stopping to speed up search
                patience_lbfgs=3
            )
            
            # Evaluate model if numerical data is available
            if df is not None:
                metrics = evaluate_model(model, df)
                ...
                results.append({
                    ...
                    'l2_errors': metrics['l2_errors'],
                    'final_unweighted_loss': history['unweighted_loss'][-1] if history['unweighted_loss'] else float('inf')
                })
            
            # Update progress bar
            pbar.update(1)
    
    # Find best configuration based on lowest loss
    if df is not None and len(results) > 0:
        best_config = None
        lowest_loss = float('inf')
        
        for result in results:
            final_loss = result['final_unweighted_loss']
            if final_loss < lowest_loss:
                lowest_loss = final_loss
                best_config = result
        
        # Calculate average R² for reporting purposes
        avg_r2 = np.mean(list(best_config['r2_scores'].values()))
        
        print("\n===== Best Weight Configuration =====")
        print(f"Weights: {best_config['weights']}")
        print(f"Final Loss: {lowest_loss:.6f}")
        print(f"Average R²: {avg_r2:.6f}")
        print(f"E value: {best_config['E']:.4f}")
        
        # Create a bar chart of R² scores for different weight configurations
        plt.figure(figsize=(12, 8))
        
        # Prepare data for plotting
        weight_labels = [f"PDE:{r['weights']['pde']}, BC:{r['weights']['bc']}, Grad:{r['weights']['grad']}" for r in results]
        loss_values = [r['final_loss'] for r in results]
        
        # Create bar chart
        plt.bar(weight_labels, loss_values)
        plt.xlabel('Weight Configuration')
        plt.ylabel('Final Loss')
        plt.title('Loss Values for Different Weight Configurations')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        
        # Save the figure
        plt.savefig(os.path.join(plots_dir, "weight_optimization_results.png"))
        plt.close()
        
        return best_config['weights']
    
    return {'pde': 1.0, 'bc': 1.0, 'grad': 0.001}  # Return default if no comparison was possible

# Function to perform hyperparameter search
def hyperparameter_search(file_path=None, max_epochs=1000, n_trials=100):
    # Create plots directory if it doesn't exist
    plots_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "efc_plots")
    if not os.path.exists(plots_dir):
        os.makedirs(plots_dir)
    ...
    learning_rates = [round(float(x), 6) for x in np.logspace(-5, -2, num=6)]
    n_weight_samples = 20

    #Loss weight configurations to try
    # loss_weights_configs = []
    # for _ in range(n_weight_samples):
    #     bc = round(log_sample(1e-4, 1e0), 4)
    #     grad = round(log_sample(1e-4, 1e0), 4)
    #     loss_weights_configs.append({
    #         'pde': 1.0,
    #         'bc': bc,
    #         'grad': grad
    # })
    ...
    (1.0, b, g)
    for b in [1.0, 0.1, 0.01, 0.001]
    for g in [1.0, 0.1, 0.01, 0.001, 0.0001]
    ]
    ...
    for layers in layers_list:
        for neurons in neurons_list:
            for learning_rate in learning_rates:
                for loss_weight in loss_weights_configs:
                    config = {
                    'layers': layers,
                    'neurons': neurons,
    ...
    if len(configs) > n_trials:
        idx = np.random.choice(len(configs), size=n_trials, replace=False)
        configs = [configs[i] for i in idx]
    total_configs = len(configs)
    # Create progress bar for all configurations
    with tqdm(total=total_configs, desc="Hyperparameter search") as pbar:
        ...
        for config in configs:
            ...
            pbar.set_description(f"Training layers={layers}, neurons={neurons}")
            ...
            model, history = train_model(
                ...
                max_epochs=max_epochs,  # Reduced for demonstration
                batch_size=512,
                use_lbfgs=True,
                lr=learning_rate,
            )
    ...
    # Find best configuration based on average R²
    if df is not None and len(results) > 0:
        best_config = None
        lowest_loss = float('inf')
...
# Function to perform hyperparameter search using Optuna
def hyperparameter_search_optuna(file_path=None, max_epochs=10, n_trials=50, num_points=5000):
    """
    Perform hyperparameter search using Optuna optimization framework.
    
    Args:
        file_path: Path to numerical data file for evaluation
        max_epochs: Maximum number of training epochs per trial
        n_trials: Number of optimization trials to run
        num_points: Number of collocation points to use
    ...
    def objective(trial, numerical_df=None):
        # Sample hyperparameters
        layers = trial.suggest_categorical("layers", [4, 6, 8])
        neurons = trial.suggest_categorical("neurons", [32, 64, 128])
        learning_rate = trial.suggest_float("learning_rate", 1e-5, 1e-2, log=True)
        
        # Sample loss weights
        pde_weight = trial.suggest_float("pde_weight", 1e-3, 1e1, log=True)
        bc_weight = trial.suggest_float("bc_weight", 1e-3, 1e1, log=True)
        grad_weight = trial.suggest_float("grad_weight", 1e-3, 1e1, log=True)
        
        # Normalize weights to make pde_weight = 1.0
        loss_weights = {
            'pde': 1.0,
            'bc': bc_weight / pde_weight,
            'grad': grad_weight / pde_weight
        }
        ...
        if numerical_df is not None:
            metrics = evaluate_model(model, numerical_df)
            
            # Report intermediate values for pruning and visualization
            trial.report(history['total_loss'][-1], step=max_epochs)
            trial.set_user_attr("E", model.E.item())
            
            for key, value in metrics['r2_scores'].items():
                trial.set_user_attr(f"r2_{key}", value)
            
            for key, value in metrics['l2_errors'].items():
                trial.set_user_attr(f"l2_{key}", value)
        
        # Return the unweighted loss as the objective to minimize
        if 'unweighted_loss' in history and history['unweighted_loss']:
            return history['unweighted_loss'][-1]
        else:
            return float('inf')
    
    print("Loading numerical data for evaluation...")
    try:
        # Load numerical data for evaluation
        if file_path is None:
            # Look for numerical data in the same directory as this script
            file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "efc_numerical.csv")
            if not os.path.exists(file_path):
        ...
        if os.path.exists(file_path):
            df = pd.read_csv(file_path)
            print(f"Loaded numerical data with {len(df)} rows.")
        else:
            print(f"Warning: Could not find numerical data file at {file_path}")
            df = None
    except Exception as e:
    ...
    # Create Optuna study
    study_name = f"pinn_optimization_{num_points}pts"
    storage_name = f"sqlite:///{study_name}.db"
    
    try:
        # Try to load existing study or create a new one
    ...
    objective_func = lambda trial: objective(trial, numerical_df=df)
    ...
    # Extract normalized loss weights
    pde_weight = 1.0  # We normalized to make pde_weight = 1.0
    bc_weight = best_params.get("bc_weight", 0.1) / best_params.get("pde_weight", 1.0)
    grad_weight = best_params.get("grad_weight", 0.001) / best_params.get("pde_weight", 1.0)
    
    # Create best configuration dictionary
    best_config = {
        'layers': best_params.get("layers", 6),
        ...
        'final_loss': best_value,
        'E': best_trial.user_attrs.get("E", float('nan'))
    }
    ...
    print(f"Final loss: {best_value:.6f}")
    if 'E' in best_config and not np.isnan(best_config['E']):
        print(f"E value: {best_config['E']:.4f}")
    ...
    try:
        ...
        fig1.write_image(os.path.join(plots_dir, "optuna_history.png"))
        
        # Plot parameter importances
        fig2 = plot_param_importances(study)
        fig2.write_image(os.path.join(plots_dir, "optuna_importances.png"))
...
if __name__ == "__main__":
    ...
    # Option 2: Find optimal loss weights first, then train with those weights
    # best_weights = find_optimal_weights(file_path=file_path, max_epochs=10)
    # model, _ = train_model(seed=1234, loss_weights=best_weights, patience_adam=1000, patience_lbfgs=5, max_epochs=10)
    # if os.path.exists(file_path):
    #     df = pd.read_csv(file_path)
    #     plot_solutions_with_numerical(model, numerical_df=df, save_dir=plots_dir)
    ...
    # Option 4: Run full hyperparameter search
    best_config = hyperparameter_search()
    
    # Option 5: Run Optuna hyperparameter search
    # best_config = hyperparameter_search_optuna(n_trials=20)
